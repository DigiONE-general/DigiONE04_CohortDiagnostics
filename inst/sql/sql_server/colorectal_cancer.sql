...
( 
  select concept_id from @vocabulary_database_schema.CONCEPT where (concept_id in (4246125,80045,192836,197500,199747,4094863,4247719,4311480,432257,432837,433143,435752,436635,436913,437798,438090,438699,438979,441800,44504380,74582,79740))
UNION  select c.concept_id
  from @vocabulary_database_schema.CONCEPT c
  join @vocabulary_database_schema.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
  WHERE c.invalid_reason is null
  and (ca.ancestor_concept_id in (4246125,80045,192836,197500,199747,4094863,4247719,4311480,432257,432837,433143,435752,436635,436913,437798,438090,438699,438979,441800,44504380,74582,79740))
